...
bool ImGui::TreeNodeEx(const char* label, ImTextureID nodeTexture, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 itemBeginPos = window->DC.CursorPos;
    ...
    // Render
    {
        ...
        //Render image
        window->DrawList->AddImage(nodeTexture, { text_pos.x ,itemBeginPos.y }, { text_pos.x + frame_height, itemBeginPos.y + frame_height });
        text_pos.x += frame_height + g.FontSize * 0.5f;
